...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const calendar = new FullCalendar.Calendar(calEl, {
        ...
        eventClick: function (info) {
            ...
            let html = `<p class="mb-2"><i class="bi bi-calendar3 me-2 text-danger"></i><strong>${escapeHtml(start)}${escapeHtml(end)}</strong></p>`;
            if (props.location) html += `<p class="mb-2"><i class="bi bi-geo-alt me-2 text-danger"></i>${escapeHtml(props.location)}</p>`;
            if (props.description) html += `<p class="mb-0 text-muted">${escapeHtml(props.description)}</p>`;
