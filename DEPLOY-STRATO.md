...
```
*.php            (alle Seiten: index.php, nachrichten.php, …)
admin/           (Verwaltungsbereich)
includes/        (Kernfunktionen)
css/  js/  images/
data/            (Inhalte – Nachrichten, Termine, Ansprechpartner …)
uploads/         (Fotos, PDFs)
...
> *Server → Versteckte Dateien anzeigen erzwingen*. Es gibt drei davon:
> im Hauptordner, in `data/` und in `uploads/` – alle drei müssen mit hoch.
...
- Ordner `data/`, `uploads/`, `uploads/galerie/`, `uploads/formulare/` → **755**
- Dateien darin → **644**
(Falls weiterhin nötig: Ordner **775**.)

## 5. Erste Schritte nach dem Upload
1. Domain im Browser öffnen → die Startseite muss erscheinen.
2. `https://deine-domain.de/admin/` öffnen → einloggen.
3. **Sofort das Passwort ändern:** Einstellungen → „Admin-Passwort ändern".
4. Unter Einstellungen die **Telefonnummer** eintragen und Inhalte pflegen.
